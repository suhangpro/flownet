function stats = flownet_train(net, fldb, getBatch, varargin)
...
if isempty(opts.train), opts.train = find(fldb.flows.set==1) ; end
if isempty(opts.val), opts.val = find(fldb.flows.set==2) ; end
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    inputs = state.getBatch(state.fldb, batch) ;
    ...
    if opts.prefetch
      ...
      state.getBatch(state.fldb, nextBatch) ;
    end
    
    if numGpus >= 1
      inputs{2} = gpuArray(inputs{2});
      inputs{4} = gpuArray(inputs{4});
      inputs{6} = gpuArray(inputs{6});
    end
